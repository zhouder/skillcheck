...
const contextBudget = defineRule(
  ...
  ({ skill, createFinding }) => {
    ...
    if (lines <= 500 && estimatedTokens <= 5_000) return [];
    const reasons = [
...
const largeResource = defineRule(
  {
    ...
    defaultSeverity: "warning",
    recommendation:
      "Remove generated assets or document how the agent should access large files selectively."
  },
  ({ skill, createFinding }) =>
    skill.files
...
const packageSize = defineRule(
  {
    ...
    defaultSeverity: "warning",
    recommendation:
      "Exclude generated output, dependencies, caches, and source material not used at runtime."
  },
  ({ skill, createFinding }) => {
    const total = skill.files.reduce((sum, file) => sum + file.size, 0);
    if (total <= 10 * 1024 * 1024 && skill.files.length <= 250) return [];
    return [
      createFinding({
...
export function estimateTokens(value: string): number {
  if (!value) return 0;
  const asciiLength = [...value].filter(
...
export const efficiencyRules: Rule[] = [contextBudget, largeResource, packageSize];
