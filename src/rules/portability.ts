...
const unicodeName = defineRule(
  {
    ...
    description:
      "The reference validator accepts Unicode names, but some clients document ASCII-only names.",
    category: "portability",
    defaultSeverity: "info",
    recommendation:
      "Prefer lowercase ASCII letters, numbers, and hyphens for maximum compatibility."
  },
  ({ skill, createFinding }) => {
...
const experimentalTools = defineRule(
  {
    ...
    defaultSeverity: "info",
    recommendation:
      "Document tool requirements in compatibility and do not rely on universal enforcement."
  },
  ({ skill, createFinding }) => {
    if (!("allowed-tools" in skill.metadata)) return [];
    return [
      createFinding({
...
const symbolicLinks = defineRule(
  {
    ...
    description:
      "Some archives, package managers, and operating systems do not preserve symbolic links.",
    category: "portability",
  ...
  ({ skill, createFinding }) => {
    ...
    for (const pattern of patterns) {
      for (const match of skill.body.matchAll(pattern)) {
        ...
        if (!matchedPath || path.isAbsolute(matchedPath) === false) continue;
        const line =
          skill.bodyStartLine + skill.body.slice(0, match.index).split(/\r?\n/u).length - 1;
        findings.push(
          createFinding({
...
export const portabilityRules: Rule[] = [
  unicodeName,
  experimentalTools,
  symbolicLinks,
  hardcodedHome
