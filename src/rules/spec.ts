...
const nameRule = defineRule(
  ...
  ({ skill, createFinding }) => {
    ...
    if (name.length > 64) problems.push(`exceeds the 64 character limit (${name.length})`);
    if (name !== name.toLowerCase()) problems.push("must be lowercase");
    if (name.startsWith("-") || name.endsWith("-"))
      problems.push("cannot start or end with a hyphen");
    if (name.includes("--")) problems.push("cannot contain consecutive hyphens");
...
const compatibilityRule = defineRule(
  ...
  ({ skill, createFinding }) => {
    if (!("compatibility" in skill.metadata)) return [];
...
const metadataRule = defineRule(
  ...
  ({ skill, createFinding }) => {
    if (!("metadata" in skill.metadata)) return [];
