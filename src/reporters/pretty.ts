...
export function prettyReport(report: AnalysisReport, options: { color?: boolean } = {}): string {
  const colors = pc.createColors(options.color ?? true);
  const lines: string[] = [colors.bold(`Skillcheck ${report.tool.version}`), ""];

  for (const finding of report.globalFindings) {
    lines.push(formatFinding(finding, colors));
  }
  if (report.globalFindings.length) lines.push("");
  ...
  for (const result of report.results) {
    ...
    lines.push(
      colors.dim(
        `${result.skillFile} | ${result.metrics.fileCount} files | ~${result.metrics.estimatedTokens} context tokens`
      )
    );
  ...
  lines.push(
    summaryText(report, colors),
    colors.dim("Run `skillcheck rules` for rule descriptions and configuration IDs.")
  );
  return `${lines.join("\n").trimEnd()}\n`;
}
...
function formatFinding(finding: Finding, colors: Colors): string {
  ...
  if (finding.help) lines.push(`         ${colors.cyan("fix:")} ${finding.help}`);
  return lines.join("\n");
}

function colorScore(value: string, score: number, colors: Colors): string {
  if (score >= 90) return colors.green(value);
  if (score >= 70) return colors.yellow(value);
  return colors.red(value);
}

function summaryText(report: AnalysisReport, colors: Colors): string {
