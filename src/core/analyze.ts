...
import { SKILLCHECK_VERSION } from "./version.js";

export { SKILLCHECK_VERSION };
...
export async function analyze(options: AnalyzeOptions = {}): Promise<AnalysisReport> {
  ...
  const globalFindings: Finding[] = discovery.issues.map((issue) =>
    createFinding({
...
async function runRules(
  skill: ParsedSkill,
  root: string,
  settings: Record<string, Severity | "off">
): Promise<Finding[]> {
...
function collectMetrics(skill: ParsedSkill): SkillMetrics {
  ...
  return {
    ...
    referenceCount: skill.references.filter(
      (reference) => !/^[a-z][a-z\d+.-]*:/iu.test(reference.destination)
    ).length,
    externalLinkCount: skill.references.filter((reference) =>
      /^https?:\/\//iu.test(reference.destination)
    ).length,
