...
type OutputFormat = "pretty" | "json" | "sarif" | "markdown" | "badge";
...
function createProgram(io: CliIo): Command {
  ...
  program
    ...
    .option(
      "-f, --format <format>",
      "pretty, json, sarif, markdown, or badge",
      parseFormat,
      "pretty"
    )
    ...
    .action(async (paths: string[], options: CheckOptions, command: Command) => {
    ...
    .action((options: { json?: boolean }) => {
      if (options.json) {
        ...
        const lines = rules.map(
          (rule) =>
            `${rule.meta.id.padEnd(34)} ${rule.meta.defaultSeverity.padEnd(7)} ${rule.meta.title}`
        );
  ...
  program
    ...
    .action(async (directory: string, options: { description?: string }) => {
...
function renderReport(report: AnalysisReport, format: OutputFormat, color: boolean): string {
  switch (format) {
    case "pretty":
...
function exitCodeFor(report: AnalysisReport, failOn: FailOn): number {
...
function parseFailOn(value: string): FailOn {
  if (["error", "warning", "never"].includes(value)) return value as FailOn;
...
function validateInitName(name: string): void {
  ...
  if (
    ...
    throw new Error(
      `Cannot initialize skill in \`${name}\`: the directory name must be lowercase letters, numbers, and single hyphens.`
    );
  }
}
