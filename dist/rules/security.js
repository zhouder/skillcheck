...
import { compactEvidence, defineRule, isInside, lineOf, readScannableFiles, regexFindings, skillFileLocation } from "./helpers.js";
const remoteExecution = defineRule({
    ...
    recommendation: "Download a pinned artifact, verify its checksum, inspect it, then execute locally."
    ...
    return regexFindings({
        ...
        create: ({ file, line, evidence }) => createFinding({
            message: "The skill executes content obtained from a remote location without a verification step.",
...
const destructiveCommand = defineRule({
    ...
    return regexFindings({
        ...
        create: ({ file, line, evidence }) => createFinding({
            message: "A command may recursively delete or overwrite data outside a bounded workspace.",
            location: { path: file.path, line, column: 1 },
            evidence,
...
const credentialAccess = defineRule({
    ...
    recommendation: "Use narrowly scoped environment variables and never print or transmit secret values."
}, async ({ skill, createFinding }) => {
    const files = await readScannableFiles(skill);
    return regexFindings({
        files,
...
const sensitiveFile = defineRule({
    ...
    category: "security",
    defaultSeverity: "error",
    recommendation: "Remove the file, rotate exposed credentials, and document required environment variables."
}, async ({ skill, createFinding }) => {
...
const escapingSymlink = defineRule({
    ...
    category: "security",
    defaultSeverity: "error",
    recommendation: "Replace external symbolic links with reviewed files inside the skill directory."
}, ({ skill, createFinding }) => skill.files.flatMap((file) => {
    if (file.kind !== "symlink" || !file.symlinkTarget)
        return [];
    const resolved = path.resolve(path.dirname(file.absolutePath), file.symlinkTarget);
    if (isInside(skill.directory, resolved))
        return [];
    return [
        createFinding({
...
}, ({ skill, createFinding }) => skill.references.flatMap((reference) => {
    if (!/^http:\/\//iu.test(reference.destination))
        return [];
