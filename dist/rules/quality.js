...
const descriptionDiscovery = defineRule({
    ...
    const hasActivationContext = /\b(when|whenever|use (?:this )?(?:skill )?(?:for|to)|trigger|task|working with)\b/iu.test(description) || /(?:当|用于|适用|使用场景|需要|任务)/u.test(description);
    if (description.length >= 60 && hasActivationContext)
        return [];
    return [
        createFinding({
...
const license = defineRule({
    ...
    description: "Published skills should tell users how bundled instructions and code may be used.",
    category: "quality",
    defaultSeverity: "info",
    recommendation: "Set the `license` field and include the referenced license file when needed."
}, ({ skill, createFinding }) => {
    if (typeof skill.metadata.license === "string" && skill.metadata.license.trim())
        return [];
    return [
        createFinding({
...
const duplicateHeadings = defineRule({
    ...
    const seen = new Map();
    const findings = [];
    visit(skill.markdown, "heading", (node) => {
        const heading = node;
        ...
        if (prior) {
            findings.push(createFinding({
                ...
                location: skillFileLocation(skill, skill.bodyStartLine + (heading.position?.start.line ?? 1) - 1, heading.position?.start.column ?? 1),
